...
void LightTextureShader::loadShader()
{
#if defined(XCGRAPHICS_DX12)
    ...
    ValidateResult(D3DCompileFromFile(L"Assets\\Shaders\\LightTexture\\LightTextureVS.hlsl", nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "LightTexture_VS", "vs_5_0", compilerFlags, 0, &m_pVS, &errors));
    ReleaseCOM(errors);
    ValidateResult(D3DCompileFromFile(L"Assets\\Shaders\\LightTexture\\LightTexturePS.hlsl", nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "LightTexture_PS", "ps_5_0", compilerFlags, 0, &m_pPS, &errors));
    ReleaseCOM(errors);
#else
    ValidateResult(ReadRawDataFromFile("Assets\\Shaders\\LightTexture\\LightTextureVS.cso", &m_pVS, m_vsSize));
    ValidateResult(ReadRawDataFromFile("Assets\\Shaders\\LightTexture\\LightTexturePS.cso", &m_pPS, m_psSize));
...
void LightTextureShader::applyShader(ID3DDeviceContext& context)
{
#if defined(XCGRAPHICS_DX12)
    //Set the pso
...
void LightTextureShader::createBufferConstants()
{
    IShader::createBufferConstants();

#if defined(XCGRAPHICS_DX12)
    m_pCBPerObject          = createBuffer(BUFFERTYPE_CBV, sizeof(cbPerObjectBuffer));
    m_pCBLightsPerFrame     = createBuffer(BUFFERTYPE_CBV, sizeof(cbLightsPerFrame));
#elif defined(XCGRAPHICS_DX11)
    m_pCBPerObject          = createBuffer(sizeof(cbPerObjectBuffer));
...
#if defined(XCGRAPHICS_DX12)
void LightTextureShader::setCBPerObject(ID3DDeviceContext& context, D3D12_GPU_DESCRIPTOR_HANDLE& gpuHandle)
{
    context.SetGraphicsRootDescriptorTable(0, gpuHandle);
}
#endif

void LightTextureShader::setCBPerObject(ID3DDeviceContext& context, cbPerObjectBuffer& cbPerObject)
{
#if defined(XCGRAPHICS_DX12)
    memcpy(m_pCBPerObject->m_cbDataBegin, &cbPerObject, sizeof(cbPerObjectBuffer));
    context.SetGraphicsRootDescriptorTable(0, m_pCBPerObject->m_gpuHandle);
#elif defined(XCGRAPHICS_DX11)
    MemCpyConstants(context, (void*)m_pCBPerObject, (void*)&cbPerObject, sizeof(cbPerObjectBuffer));
    context.PSSetConstantBuffers(0, 1, &m_pCBPerObject);
    context.VSSetConstantBuffers(0, 1, &m_pCBPerObject);
#elif defined(XCGRAPHICS_GNM)
    createBuffer<cbPerObjectBuffer>(context, &m_pPerObject, m_pCBPerObject);
    ::memcpy(m_pPerObject, &cbPerObject, sizeof(cbPerObjectBuffer));

    context.setConstantBuffers(sce::Gnm::kShaderStageVs, 0, 1, &m_pCBPerObject);
    context.setConstantBuffers(sce::Gnm::kShaderStagePs, 0, 1, &m_pCBPerObject);
#endif
}

void LightTextureShader::setCBLightsPerFrame(ID3DDeviceContext& context, cbLightsPerFrame& lightsPerFrame)
{
#if defined(XCGRAPHICS_DX12)
...
void LightTextureShader::setTexture2D(ID3DDeviceContext& context, D3DShaderResourceView* tex)
{
#if defined(XCGRAPHICS_DX12)
    if (tex)
    {
        context.SetGraphicsRootDescriptorTable(2, tex->m_gpuHandle);
    }
#elif defined(XCGRAPHICS_DX11)
    if(tex)
    {
...
void LightTextureShader::reset(ID3DDeviceContext& context)
{
    m_pTexture2DDiffuseMap = nullptr;
    setTexture2D(context, m_pTexture2DDiffuseMap);
}

void LightTextureShader::destroy()
{
#if defined(XCGRAPHICS_DX12)
    m_pCBPerObject->Release();
    m_pCBLightsPerFrame->Release();
#elif defined(XCGRAPHICS_DX11)
    ReleaseCOM(m_pCBLightsPerFrame);
