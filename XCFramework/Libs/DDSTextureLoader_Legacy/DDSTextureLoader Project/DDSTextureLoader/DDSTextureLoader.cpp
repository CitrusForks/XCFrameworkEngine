...
{
    OutputDebugString(L"[Way 1]");
    if ( !pDev || !szFileName || !ppSRV )
        return E_INVALIDARG;
    OutputDebugString(L"[Way 2]Args are good");
    ...
    OutputDebugString(L"[CreateDDSTextureFromFile] On the way to loadtex");
    if(FAILED(hr))
    {
        OutputDebugString(L"[CreateDDSTextureFromFile] FAILED--------------");
        SAFE_DELETE_ARRAY( pHeapData );
        return hr;
    }

    OutputDebugString(L"[CreateDDSTextureFromFile] SUCCESS--------------");
