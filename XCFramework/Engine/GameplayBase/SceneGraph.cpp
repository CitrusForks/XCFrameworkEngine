...
void SceneGraph::Update(f32 dt)
{
    ...
    for (auto& sceneNode : m_sceneGraph->GetRootNode()->GetNodesMutable())
    {
        ...
        {
            RequestRemoveActor(sceneNode->GetBaseObjectId());
        }
        else if (sceneNode->IsActorReady() && sceneNode->GetActorState() == IActor::ActorState_Loaded)
        {
            sceneNode->Update(dt);
        }
        else if (sceneNode->GetActorState() == IActor::ActorState_PreLoaded)
        {
            //Request a load here.
            RequestLoadActor(sceneNode);

            //Also register it for rendering
            XCGraphics& graphicsSystem = SystemLocator::GetInstance()->RequestSystem<XCGraphics>("GraphicsSystem");
            graphicsSystem.GetRenderingPool().AddRenderableObject((IRenderableObject*)sceneNode, sceneNode->GetBaseObjectId());
        }
        else
        {
            sceneNode->UpdateState();
        }
    }
}
...
{
    SceneGraphPendingTasks::IPendingTask* task = XCNEW(SceneGraphPendingTasks::PendingTaskAdd)(SceneGraphPendingTasks::PENDINGTASK_ADD, actor);

    //Set the actor state to loading to avoid multiple load requests.
    actor->SetActorState(IActor::ActorState_Loading);
...
bool SceneGraph::IsActorReady(i32 actorId)
{
    ...
    {
       return m_GameObjects[actorId]->IsActorReady();
    }
